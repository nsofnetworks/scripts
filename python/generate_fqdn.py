...
Optional parameters:
1. --deprovision to remove the DNS records you added (You should include the domain name if given):
API_KEY_ID=key-** API_KEY_SECRET=** python3 generate_fqdn.py --domain-name x.x --deprovision
2. --dryrun to run the script without actually creating the records
"""
import functools
import os
import re
...
def modify_aliases(method, ne_id, alias):
    ...
    if not response.ok:
        print("ERROR: %s", response.json()['detail'])
...
def generate_fqdns(domain_name=None, deprovisioning=False, dry_run=False):
...
if __name__ == '__main__':
    ...
    if '--domain-name' in sys.argv:
        try:
            ...
            sys.exit()
    generate_fqdns(
        domain_name=domain_name, deprovisioning=deprovision, dry_run=dry_run)
